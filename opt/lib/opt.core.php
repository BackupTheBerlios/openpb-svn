...
opt_console.document.close();
	...
	function optErrorMessage($tpl, $type, $message, $code)
	{
		...
		{
			echo '<br/><b>'.$n_type.' warning #'.$code.':</b> '.$message.' <i>Generated by OPT method `'.$dFunction.'` called in '.$dFile.' on line '.$dLine.'</i><br/>';
		}
		elseif($type == E_USER_ERROR)
		{
			// Send the exception
			$exception = new optException($message, $code, $n_type, $dFile, $dLine, $dFunction);
