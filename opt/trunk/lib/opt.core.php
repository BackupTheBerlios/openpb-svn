...
opt_console.document.write(\'<title>OPT Debug Console</title>\');
...
opt_console.document.write(\'</head>\');
opt_console.document.write(\'<body>\');
opt_console.document.write(\'<h1>OPT Debug Console</h1>\');
opt_console.document.write(\'<table id="info">\');
';

	foreach($config as $name => $value)
	{
		echo 'opt_console.document.write(\'<tr>\');
...
echo 'opt_console.document.write(\'</table>\');';
...
';
	if(sizeof($templates) > 0)
	{
		foreach($templates as $data)
		{
			if($data['cached'] == true)
			{
				echo 'opt_console.document.write(\'<tr class="cached">\');';		
			}
			else
			{
				echo 'opt_console.document.write(\'<tr>\');';
			}
			echo 'opt_console.document.write(\' <td>'.addslashes($data['template']).'</td>\');
...
opt_console.document.write(\'</tr>\');';	
	...
	{
			echo 'opt_console.document.write(\' <td colspan="4">No template info provided because of "performance tuning" option enabled.</td>\');
opt_console.document.write(\'</tr>\');';
	}
echo '
opt_console.document.write(\'</tbody>\');
...
opt_console.document.close();
	...
	function optErrorMessage($tpl, $type, $message, $code, $filename = NULL)
	{
		...
		{
			echo '<div class="warning opt">
			<p class="message"><strong> '.$n_type.' warning #'.$code.'</strong>:  '.$message.'</p>';
			if($code >= 100)
			{
				echo '<p class="location">Method: "<em>'.$dFunction.'</em>"; Template: "<em>'.$filename.'</em>"; File: "<em>'.$dFile.'</em>"; Line: "<em>'.$dLine.'</em>"</p>';
			}
			else
			{
				echo '<p class="location">Method: "<em>'.$dFunction.'</em>"; File: "<em>'.$dFile.'</em>"; Line: "<em>'.$dLine.'</em>"</p>';			
			}
			echo '</div>';
		}
